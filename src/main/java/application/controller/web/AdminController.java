...
public class AdminController extends BaseController {
    ...
    public String chart(Model model) {
        ...
        List<ChartLabelDataVM> countProductBySold= categoryService.countProductSold();
        ChartVM chartVM= new ChartVM();
        chartVM.setLabelDataVMList(countProductByCategory);

      // da ban
        ChartVM chartVM2= new ChartVM();
        chartVM2.setLabelDataVMList(countProductBySold);

        vm.setCountProductByCategory(chartVM);
        vm.setCountProductSold(chartVM2);
        vm.setLayoutHeaderAdminVM(this.getLayoutHeaderAdminVM());
        model.addAttribute("vm",vm);
